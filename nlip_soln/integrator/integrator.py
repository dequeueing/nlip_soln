...
import nlip_soln.integrator.config as cfg


class IntegratorApplication(server.NLIP_Application):
    def startup(self):
...
class IntegratorSession(server.NLIP_Session):
...
app = server.setup_server(IntegratorApplication())
